-- (O) 1. 영어영문학과(학과코드 002) 학생들의 
-- 학번과 이름, 입학 년도를 입학 년도가 빠른 순으로 표시하는 SQL 문장을 작성하시오. 
...
SELECT DEPARTMENT_NO 학번, STUDENT_NAME 이름, EXTRACT(YEAR FROM ENTRANCE_DATE) 입학년도
FROM TB_STUDENT
WHERE DEPARTMENT_NO = 002
ORDER BY EXTRACT(YEAR FROM ENTRANCE_DATE);

-- (O) 2. 춘 기술대학교의 교수 중 이름이 세 글자가 아닌 교수가 한 명 있다고 한다. 
...
-- (O) 4. 교수들의 이름 중 성을 제외한 이름만 출력하는 SQL 문장을 작성하시오. 
-- 출력 헤더는 "이름" 이 찍히도록 한다. (성이 2자인 경우 교수는 없다고 가정하시오)
SELECT SUBSTR(PROFESSOR_NAME,2) "이름"
FROM TB_PROFESSOR;


-- (O) 5. 춘 기술대학교의 재수생 입학자를 구하려고 한다. 어떻게 찾아낼 것인가? 
...
-- (O) 6. 2020년 크리스마스는 무슨 요일인가?
SELECT TO_CHAR(TO_DATE(20201225), 'DAY')
FROM DUAL;


-- (O) 7. TO_DATE('99/10/11', 'YY/MM/DD'), TO_DATE('49/10/11', 'YY/MM/DD') 은 각각 몇 년 몇 월 몇 일을 의미할까? 
...
-- (O) 8. 춘 기술대학교의 2000년도 이후 입학자들은 학번이 A로 시작하게 되어있다. 
...
-- (O) 9. 학번이 A517178 인 한아름 학생의 학점 총 평점을 구하는 SQL 문을 작성하시오. 
...
-- (O) 10. 학과별 학생수를 구하여 "학과번호", "학생수(명)" 의 형태로
...
-- (O) 11. 지도 교수를 배정받지 못한 학생의 수를 몇 명 정도 되는지 알아내는 SQL 문을 작성하시오.
SELECT COUNT(*)
FROM TB_STUDENT
WHERE COACH_PROFESSOR_NO IS NULL;


-- (O) 12. 학번이 A112113 인 김고운 학생의 년도 별 평점을 구하는 SQL 문을 작성하시오. 
...
SELECT TERM_NO "년도", ROUND(AVG(POINT),1)
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY TERM_NO
HAVING ROUND(AVG(POINT),1);
...
-- 년도 별 누적 평점
SELECT TERM_NO, ROUND(AVG(POINT),1), ROUND(AVG(SUM(POINT))),1
FROM TB_GRADE
GROUP BY TERM_NO, ROUND(AVG(POINT),1);
