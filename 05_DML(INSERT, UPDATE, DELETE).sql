...
-- INSERT하고자 하는 칼럼이 모든 칼럼인 경우 컬럼명 생략 가능
-- 단, 컬럼의 순서를 지켜서 VALUES에 값을 기입해야함!

INSERT INTO EMPLOYEE2
VALUES(900, '장채헌', '901123-1234567', 'jang_ch@kh.or.kr', 
'01012341234', 'D1', 'J7', 'S1', 9000000, 0.8, 200, SYSDATE, NULL, 'N');
...
VALUES(900, '장채헌', '901123-1234567', 'jang_ch@kh.or.kr', '01012341234', 'D1', 'J7', 'S1', 9000000);

SELECT * FROM EMPLOYEE2
WHERE EMP_ID = 900;

COMMIT; -- 수행된 DML 결과를 DB에 반영 -- 실질적으로 반영되는 것
ROLLBACK; -- 마지막 COMMIT 시점으로 돌아감

--------------------------------------------------------------------------------

-- (참고) INSERT 할 때 VALUES 대신 서브쿼리 사용 가능
CREATE TABLE EMP_01(
    EMP_ID NUMBER,
...
-- 서브쿼리(SQL 내부에 포함된 SELECT문)를 이용한 삽임(VALUES 생략)
...
-- * 여러 컬럼을 한번에 수정할 시 콤마(,)로 칼럼을 구분
-- D9/전략기획팀 -> D0/전략기획2팀으로 수정
UPDATE DEPARTMENT2
SET DEPT_TITLE = '전략기획2팀', DEPT_ID = 'D0'
...
-- 노옹철, 전형돈, 정중하, 하동운 사원의 급여와 보너스를 유재식 사원의 급여, 보너스가 같게 변경하는 UPDATE문을 작성하시오
...
-- EMPLOYEE2 테이블에서 '장채헌' 사원 정보 조회
SELECT *
FROM EMPLOYEE2
WHERE EMP_NAME = '장채헌';

-- EMPLOYEE2 테이블에서 이름이 '장채헌'인 사원 정보 삭제
