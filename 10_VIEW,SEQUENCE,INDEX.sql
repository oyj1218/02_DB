...
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) JOIN JOB USING (JOB_CODE);
--> 자주 사용하는데 매번 쓰기 힘듦 -> VIEW를 생성
...
--> 각자 이니셜 계정에 VIEW 생성 권한을 부여(관리자 계정 사용)
GRANT CREATE VIEW TO oyj; -- Grant을(를) 성공했습니다.
...
-- 뷰를 이용한 INSERT
INSERT INTO V_DCOPY2 VALUES('D0','L3'); -- 1행이(가) 삽입되었습니다

-- 삽입 확인
SELECT * FROM V_DCOPY2;
--> 가상의 테이블인 VIEW에 데이터 삽입 가능? NO
...
-- ORA-01400: NULL을 ("JHS","DEPT_COPY2","DEPT_TITLE") 안에 삽입할 수 없습니다
-- == INSERT INTO DEPT_COPOY2 VALUES('D0',NULL,'L3');

-- VIEW을 이용한 INSERT 시 원본 테이블에 삽입이 된다
--> 원본 테이블 삽입 시 VIEW INSERT 구문이 미포함된 컬럼에는 NULL이 저장된다
--> 그런데 DEPT_TITLE 컬럼에 NOT NULL 제약조건이 설정되어 있어서 오류 발생!
--> VIEW를 이용한 INSERT 실패

-- 결론 : VIEW 가지고 DML 하지말자
...
CREATE SEQUENCE SEQ_EMP_ID START WITH 223 INCREMENT BY 5;

-- NEXTVAL 호출 없이 CURRVAL를 호출하면 발생하는 오류 확인
...
-- EMPLOYEE_COPY4 테이블의 EMP_NAME 컬럼에 INDEX 생성(ECOPY4_NAME_IDX)
CREATE INDEX ECOPY4_NAME_IDX ON EMPLOYEE_COPY4(EMP_NAME);
...
WHERE TEST_ID = 'TEST50000'; -- 0.014

-- 인덱스 사용
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 50000; -- 0.002
